...
- `src/routes/login/+page.svelte` — three sign-in paths: "Continue with Google" (social OAuth), auto-prompted Google One Tap (only when `PUBLIC_GOOGLE_CLIENT_ID` is set + in browser), and "Sign in with a passkey" (WebAuthn; hidden where the browser lacks WebAuthn). Passkeys are *registered* in `/settings`, not here.
